 ...
 *******************************************************/
static const char *MESH_TAG = "NODO_MESH";
...
void esp_mesh_p2p_tx_main(void *arg)
{
    ...
    ESP_LOGI(MESH_TAG, "Tarea TX iniciada. Esperando datos en 'mesh_tx_queue'...");

    while (is_running) {
        // 1. BLOQUEANTE: Espera hasta que llegue un mensaje a la cola
        if (xQueueReceive(mesh_tx_queue, queue_msg, portMAX_DELAY) == pdTRUE) {
            
            int msg_len = strlen(queue_msg) + 1;  // El +1 incluye el '\0'
            data.size = msg_len;
            data.data = (uint8_t *)queue_msg; // Usamos directamente el mensaje de la cola

            // 2. LÓGICA DE ENVÍO SEGÚN EL ROL DEL NODO
            if (!esp_mesh_is_root()) {
                /* --- CASO NODO HIJO: Enviar datos hacia el ROOT --- */
                // Al pasar NULL como dirección de destino, el stack Mesh 
                // lo encamina automáticamente hacia el nodo ROOT.
                err = esp_mesh_send(NULL, &data, MESH_DATA_P2P, NULL, 0);
                
                if (err == ESP_OK) {
                    ESP_LOGI(MESH_TAG, "HIJO: Mensaje enviado al ROOT: '%s'", queue_msg);
                } else {
                    ESP_LOGE(MESH_TAG, "HIJO: Error enviando al ROOT: 0x%x", err);
                }
            } 
            else {
                /* --- CASO NODO ROOT: Distribuir a todos los nodos --- */
                esp_mesh_get_routing_table((mesh_addr_t *) &route_table,
                                           CONFIG_MESH_ROUTE_TABLE_SIZE * 6, &route_table_size);

                ESP_LOGW(MESH_TAG, "ROOT: Reenviando alerta a %d nodos...", route_table_size);

                for (int i = 0; i < route_table_size; i++) {
                    err = esp_mesh_send(&route_table[i], &data, MESH_DATA_P2P, NULL, 0);
                    if (err != ESP_OK) {
                        ESP_LOGE(MESH_TAG, "ROOT: Error enviando a "MACSTR": 0x%x", 
