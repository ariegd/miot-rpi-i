...
#define BUFSIZE 40
    ...
    ESP_LOGI(TAG, "!!! TAREA CLIENTE INICIADA CON URI: %s !!!", server_uri);
...
void coapc_start(void)
{
// --- ELIMINAR O COMENTAR ESTAS LÍNEAS ---
    // El Mesh o el app_main ya inician NVS y Netif
    // ESP_ERROR_CHECK( nvs_flash_init() );
    // ESP_ERROR_CHECK(esp_netif_init());
    // ESP_ERROR_CHECK(esp_event_loop_create_default());

    // --- CRÍTICO: ELIMINAR example_connect() ---
    // ESP_ERROR_CHECK(example_connect());
    
    // Solo creamos la tarea de CoAP. 
    // Nota: Deberás añadir lógica en la tarea para esperar a tener IP 
    // antes de intentar enviar nada, o iniciar esta tarea MÁS TARDE.
