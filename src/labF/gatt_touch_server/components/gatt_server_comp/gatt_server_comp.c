...
// Usamos el nombre definido en Kconfig
#define SAMPLE_DEVICE_NAME          CONFIG_DEVICE_NAME_GATT
...
// Definiciones para la construcción dinámica del nombre
#define DEVICE_NAME     CONFIG_DEVICE_NAME_GATT
#define DEVICE_NAME_LEN (sizeof(DEVICE_NAME) - 1)

// Función para construir el paquete RAW correctamente
static void config_adv_raw_dynamic(void) {
    uint8_t raw_adv_data_temp[31];
    uint8_t idx = 0;

    // 1. Flags (3 bytes)
    raw_adv_data_temp[idx++] = 0x02;
    raw_adv_data_temp[idx++] = ESP_BLE_AD_TYPE_FLAG;
    raw_adv_data_temp[idx++] = 0x06;

    // 2. TX Power (3 bytes)
    raw_adv_data_temp[idx++] = 0x02;
    raw_adv_data_temp[idx++] = ESP_BLE_AD_TYPE_TX_PWR;
    raw_adv_data_temp[idx++] = 0xEB;

    // 3. Service UUID (4 bytes)
    raw_adv_data_temp[idx++] = 0x03;
    raw_adv_data_temp[idx++] = ESP_BLE_AD_TYPE_16SRV_CMPL;
    raw_adv_data_temp[idx++] = 0xFF;
    raw_adv_data_temp[idx++] = 0x00;

    // 4. Nombre Completo (Dinámico)
    // Longitud = caracteres del nombre + 1 byte del tipo (0x09)
    raw_adv_data_temp[idx++] = (uint8_t)(DEVICE_NAME_LEN + 1);
    raw_adv_data_temp[idx++] = ESP_BLE_AD_TYPE_NAME_CMPL;
    memcpy(&raw_adv_data_temp[idx], DEVICE_NAME, DEVICE_NAME_LEN);
    idx += DEVICE_NAME_LEN;

    esp_ble_gap_config_adv_data_raw(raw_adv_data_temp, idx);
}
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    #ifdef CONFIG_SET_RAW_ADV_DATA
            // IMPLEMENTACIÓN SOLUCIÓN 2: Llamada a la función dinámica
            config_adv_raw_dynamic();
            /*
...
{
    xTaskCreate(&gatt_server_task, "gatt_server_task", 2048, NULL, 1, NULL);
}
